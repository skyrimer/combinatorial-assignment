...
def solve_advanced_case():
    ...
    ranks, top_2, weights = calculate_parameters(students, courses, preferences, credits)

    # 2. Model Setup
    m = gp.Model("CourseAllocation_AdvancedCase")

    # Variables
    x = {(i, j): m.addVar(vtype=GRB.BINARY, name=f"x_{i}_{j}") for i in students for j in courses}
    ...
    total_satisfaction = gp.quicksum(weights[j] * (6 - ranks[(i, j)]) * x[(i, j)]
                                     for i in students for j in preferences[i])

    # Student 8 Satisfaction Expression
    student_8_satisfaction = gp.quicksum(weights[j] * (6 - ranks[(8, j)]) * x[(8, j)]
    ...
    for j in courses:
        m.addConstr(gp.quicksum(x[(i, j)] for i in students) <= capacities[j] + n[j], name=f"Cap_{j}")
    ...
    # C2: Credit Limit (<= 40)
    for i in students:
        m.addConstr(gp.quicksum(credits[j] * x[(i, j)] for j in courses) <= 40, name=f"Credit_{i}")
    ...
    for i in students:
        m.addConstr(gp.quicksum(x[(i, j)] for j in top_2[i]) >= l[i], name=f"Link_Top2_{i}")
    ...
    if m.status == GRB.OPTIMAL:
        print_solution(m, students, courses, x, weights, ranks, credits, "Advanced Case")
        ...
        # Objective Breakdown
        total_sat_val = sum(weights[j] * (6 - ranks[(i, j)]) * x[(i, j)].X
                            for i in students for j in preferences[i] if x[(i, j)].X > 0.5)
        s8_sat_val = sum(weights[j] * (6 - ranks[(8, j)]) * x[(8, j)].X
                         for j in preferences[8] if x[(8, j)].X > 0.5)
