def calculate_parameters(students, courses, preferences, credits):
    ...
    # [cite_start]2. Top-2 Sets T2_i: The set of the first two courses for each student
    top_2 = {i: set(preferences[i][:2]) for i in students}

    # [cite_start]3. Credit Weights z_j: Credit / 10
    weights = {j: credits[j] / 10 for j in courses}

    return ranks, top_2, weights


def print_solution(m, students, courses, x, weights, ranks, credits_dict, scenario_name="Solution"):
    """
    ...
    if m.status == gp.GRB.OPTIMAL:
        ...
        for i in students:
            ...
            # Calculate satisfaction for this student
            satisfaction = sum(weights[j] * (6 - ranks[(i, j)]) for j in assigned if (i, j) in ranks)
            total_credits = sum(credits_dict[j] for j in assigned)
            print(f"  Student {i:2}: {assigned} (Sat: {int(satisfaction)}, Credits: {total_credits})")
    else:
